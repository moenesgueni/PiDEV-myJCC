...
public class GererPhotographiesFXMLController implements Initializable {
    ...
    File selectedFile;
    int CurrentUserId = UserSession.getID_User();//5
    private int idCurrentUserGalerie = 1 ;// gs.afficherGalerie(CurrentUserId).getID_Galerie(); //idCurrentGalerie (Sponsor peut y accéder depuis liste galeries)
    Type CurrentUserRole = UserSession.getRole();//PHOTOGRAPHE
    ...
    public void saveSignature() {
        try {
            ...
            ImageIO.write(SwingFXUtils.fromFXImage(snapshot, null), "png", tempFilePath.toFile());
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
